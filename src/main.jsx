...
function App() {
  ...
  const [personnelEmployees, setPersonnelEmployees] = useState(() => {
    try {
      const saved = JSON.parse(localStorage.getItem("dietexPersonnelEmployees") || "null");
      return Array.isArray(saved) && saved.length ? saved : PERSONNEL_EMPLOYEES;
    } catch {
  ...
  useEffect(() => {
    localStorage.setItem("dietexPersonnelEmployees", JSON.stringify(personnelEmployees));
  }, [personnelEmployees]);
...
const tourColumns = Object.entries(
  ...
  const setEmployeeToSection = (employeeName, sectionName) => {
    setPersonalPlan((prev) => {
      ...
      const allZones = [
        ...PERSONNEL_SECTIONS.map((section) => section.name),
        "urlaub",
        "za",
        "krank",
      ];
  ...
  const getActiveEmployees = () => personnelEmployees.filter((e) => getEmployeeStatus(e.name) === "anwesend");
  ...
  const copyWholePersonalDay = () => {
    ...
    setPersonalPlan((prev) => {
      ...
      PERSONNEL_SHIFTS.forEach((shift) => {
        const sourceKey = `${copyPersonalDate}_${shift.key}`;
        const targetKey = `${personalDate}_${shift.key}`;
        if (prev[sourceKey]) {
  ...
  const addPersonnelEmployee = () => {
    ...
    if (personnelEmployees.some((e) => e.name.toLowerCase() === name.toLowerCase())) {
    ...
    setPersonnelEmployees((prev) =>
      [...prev, { name, hours }].sort((a, b) => String(a.name).localeCompare(String(b.name), "de", { numeric: true }))
    );
    setNewEmployeeName("");
    setNewEmployeeHours("");
  };
  ...
  const editPersonnelEmployee = (oldName) => {
    const emp = personnelEmployees.find((e) => e.name === oldName);
    if (!emp) return;
    ...
    if (cleanName !== oldName && personnelEmployees.some((e) => e.name.toLowerCase() === cleanName.toLowerCase())) {
  ...
  const deletePersonnelEmployee = (name) => {
    ...
    setPersonnelEmployees((prev) => prev.filter((e) => e.name !== name));
            ...
            <div className="rounded-3xl border bg-white p-5 shadow-sm">
              ...
              <div className="grid gap-5 lg:grid-cols-[310px_1fr]">
                <aside className="rounded-3xl border bg-slate-50 p-3">
                  <h3 className="mb-2 text-lg font-black">Mitarbeiterpool</h3>
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      const zoneEmployees = getSortedEmployees(
                        personnelEmployees.filter((emp) => {
                          const assignment = getEmployeeAssignment(emp.name);

                          if (zone.key === "pool") {
                            return !assignment && getEmployeeStatus(emp.name) === "anwesend";
                          }
                                ...
                                <div className="flex items-center justify-between gap-1">
                                  ...
                                  <span className="flex gap-1">
                                    <button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        editPersonnelEmployee(emp.name);
                                      }}
                                    ...
                                    <button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        deletePersonnelEmployee(emp.name);
                                      }}
                    ...
                    <div key={group.name} className="rounded-2xl border bg-white p-2">
                      ...
                      <div className="grid auto-rows-min gap-2 xl:grid-cols-3 2xl:grid-cols-6">
                        {group.sections.map((sectionName) => {
                          const section = PERSONNEL_SECTIONS.find((s) => s.name === sectionName);
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="space-y-1">
                                {assigned.map((name) => {
                                  const emp = personnelEmployees.find((e) => e.name === name);
                                  return (
                                    <div
                                      key={name}
                                      draggable
