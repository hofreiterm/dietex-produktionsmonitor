...
function App() {
  ...
  async function archiveFinishedOrders(orderIds = null) {
    ...
    await supabase.from("orders").update({ status: "auf_tour" }).in("id", ids);
  ...
  const sortedTourRows = [...tourRows].sort((a, b) => {
    const ta = Number(a.order?.tour_number || 999);
    const tb = Number(b.order?.tour_number || 999);
    return ta - tb;
  });
            ...
            <div className="rounded-3xl border bg-white p-5 shadow-sm">
              ...
              <div className="overflow-hidden rounded-2xl border">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {[...sortedOrders.filter(o => o.status !== 'auf_tour'), ...sortedTourRows.map(r => r.order)].map((order, index) => {
