...
const PERSONNEL_EMPLOYEES = [
  { name: "Janos", hours: 40 }, { name: "Arpi", hours: 40 }, { name: "Dida", hours: 40 }, { name: "Laszlo", hours: 40 },
  { name: "Mate", hours: 40 }, { name: "Milena", hours: 40 }, { name: "Manu K.", hours: 40 }, { name: "Ibolya", hours: 30 },
  { name: "Gabriella", hours: 20 }, { name: "Olga", hours: 40 }, { name: "Attila", hours: 40 }, { name: "Ahmed", hours: 40 },
  { name: "Norbert", hours: 40 }, { name: "Carina", hours: 24 }, { name: "Krisztian", hours: 40 }, { name: "Petra", hours: 40 },
  { name: "Peter K.", hours: 40 }, { name: "Gabi", hours: 24 }, { name: "Walid", hours: 40 }, { name: "Edina N.", hours: 30 },
  { name: "Laszlo Chef", hours: "" }, { name: "Edina", hours: 40 }, { name: "Kinga", hours: 20 }, { name: "Lumi", hours: 30 },
  { name: "Martina", hours: 30 }, { name: "Anita", hours: 40 }, { name: "Katalin", hours: 40 }, { name: "Mari H.", hours: 12 },
  { name: "Csaba", hours: 20 }, { name: "Barbara", hours: 20 }, { name: "Renate", hours: 40 }, { name: "Margret", hours: 40 },
  { name: "Nicole", hours: 20 }, { name: "Brigitte", hours: 27 },
];


function Button({ children, active, className = "", ...props }) {

  function personalKey(date = personalDate, shift = personalShift) { return `${date}_${shift}`; }
  function currentPersonalPlan() { return personalPlan[personalKey()] || {}; }
  function employeesInSection(sectionName) { return currentPersonalPlan()[sectionName] || []; }
  function employeeAssignment(employeeName) { const plan = currentPersonalPlan(); for (const section of Object.keys(plan)) if ((plan[section] || []).includes(employeeName)) return section; return null; }
  function setEmployeeToSection(employeeName, sectionName) {
    setPersonalPlan((prev) => { const key = personalKey(); const current = prev[key] || {}; const next = {}; PERSONNEL_SECTIONS.forEach((s) => { next[s.name] = (current[s.name] || []).filter((n) => n !== employeeName); }); if (sectionName !== "pool") next[sectionName] = [...(next[sectionName] || []), employeeName]; return { ...prev, [key]: next }; });
  }
  function targetForSection(section) { if (section.target.flexible) return null; return section.target[personalShift] ?? section.target.default ?? 0; }
  function sectionColor(section) { const target = targetForSection(section); if (target === null) return "border-slate-300 bg-slate-50"; const actual = employeesInSection(section.name).length; if (actual < target) return "border-red-300 bg-red-50"; if (actual > target) return "border-yellow-300 bg-yellow-50"; return "border-green-300 bg-green-50"; }
  function cycleEmployeeStatus(name) { const vals = ["anwesend", "krank", "urlaub", "frei"]; const cur = employeeStatus[name] || "anwesend"; const next = vals[(vals.indexOf(cur) + 1) % vals.length]; setEmployeeStatus((prev) => ({ ...prev, [name]: next })); }
  function employeeStatusLabel(name) { return employeeStatus[name] || "anwesend"; }
  function activeEmployees() { return PERSONNEL_EMPLOYEES.filter((e) => employeeStatusLabel(e.name) === "anwesend"); }
  function unassignedEmployees() { return activeEmployees().filter((e) => !employeeAssignment(e.name)); }
  function clearPersonalPlan() { if (!confirm("Personalplanung für diese Schicht wirklich leeren?")) return; setPersonalPlan((prev) => { const next = { ...prev }; delete next[personalKey()]; return next; }); }
  function copyPreviousShift() { const idx = PERSONNEL_SHIFTS.findIndex((s) => s.key === personalShift); const prevShift = idx > 0 ? PERSONNEL_SHIFTS[idx - 1].key : "07-12"; const source = personalPlan[`${personalDate}_${prevShift}`]; if (!source) return alert("Für die vorherige Schicht ist kein Plan vorhanden."); setPersonalPlan((prev) => ({ ...prev, [personalKey()]: source })); }
  function printPersonalPlan() {
    const plan = currentPersonalPlan();
    const rows = PERSONNEL_SECTIONS.map((s) => { const names = plan[s.name] || []; const target = targetForSection(s); return `<tr><td>${s.name}</td><td>${target === null ? "bei Bedarf" : target}</td><td>${names.join(", ")}</td></tr>`; }).join("");
    const html = `<html><head><title>Personalplanung</title><style>body{font-family:Arial;padding:22px;color:#111827}h1{margin:0 0 8px}.meta{margin-bottom:18px;color:#4b5563}table{width:100%;border-collapse:collapse;font-size:13px}th{background:#e5e7eb;text-align:left}th,td{border:1px solid #cbd5e1;padding:8px;vertical-align:top}</style></head><body><h1>DieTex Personalplanung</h1><div class="meta">Datum: ${personalDate} | Schicht: ${PERSONNEL_SHIFTS.find((s) => s.key === personalShift)?.label}</div><table><thead><tr><th>Bereich</th><th>Soll</th><th>Mitarbeiter</th></tr></thead><tbody>${rows}</tbody></table><script>window.onload=()=>window.print();</script></body></html>`;
    const win = window.open("", "_blank"); if (!win) return alert("Druckfenster wurde vom Browser blockiert."); win.document.open(); win.document.write(html); win.document.close();
  }
...
function App() {
  ...
  const [personalPlan, setPersonalPlan] = useState(() => { try { return JSON.parse(localStorage.getItem("dietexPersonalPlan") || "{}"); } catch { return {}; } });
  const [employeeStatus, setEmployeeStatus] = useState(() => { try { return JSON.parse(localStorage.getItem("dietexEmployeeStatus") || "{}"); } catch { return {}; } });
  ...
  const copyPreviousShift = () => {
    ...
    const key = `${personalDate}_${previousShift}`;
    const source = personalPlan[key];
    if (!source) {
      window.alert("Für die vorherige Schicht ist kein Plan vorhanden.");
      return;
    }
    const currentKey = `${personalDate}_${personalShift}`;
    setPersonalPlan((prev) => ({ ...prev, [currentKey]: source }));
  };

  const clearPersonalPlan = () => {
  ...
  useEffect(() => { localStorage.setItem("dietexPersonalPlan", JSON.stringify(personalPlan)); }, [personalPlan]);
  useEffect(() => { localStorage.setItem("dietexEmployeeStatus", JSON.stringify(employeeStatus)); }, [employeeStatus]);
        ...
        {!fixedView && (
          <nav className="mb-5 flex flex-wrap justify-center gap-2">
            {[
              ...
              ["leitung", "Produktionsleitung"], ["personalplanung", "Personalplanung"],
            ].map(([key, label]) => (
          ...
          <section className="space-y-5">
            <div className="rounded-3xl border bg-white p-5 shadow-sm">
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                <div><h2 className="text-2xl font-black">Personalplanung</h2><p className="text-slate-500">Tagesplanung per Drag & Drop für den Produktionsleiter.</p></div>
                <div className="flex flex-wrap gap-2">
                  <Input type="date" value={personalDate} onChange={(e) => setPersonalDate(e.target.value)} />
                  {PERSONNEL_SHIFTS.map((shift) => <Button key={shift.key} active={personalShift === shift.key} onClick={() => setPersonalShift(shift.key)}>{shift.label}</Button>)}
                  <Button onClick={copyPreviousShift}>Vorherige Schicht übernehmen</Button>
                  <Button onClick={clearPersonalPlan}>Leeren</Button>
                  <Button className="bg-blue-700 text-white" onClick={printPersonalPlan}>Drucken</Button>
                </div>
              </div>
              <div className="grid gap-5 lg:grid-cols-[310px_1fr]">
                <aside className="rounded-3xl border bg-slate-50 p-4" onDragOver={(e) => e.preventDefault()} onDrop={() => dragEmployee && setEmployeeToSection(dragEmployee, "pool")}>
                  <h3 className="mb-3 text-xl font-black">Mitarbeiterpool</h3>
                  <div className="mb-4 rounded-2xl border bg-white p-3"><div className="mb-2 text-sm font-bold text-slate-500">Nicht eingeteilt</div><div className="space-y-2">
                    {unassignedEmployees().map((emp) => <div key={emp.name} draggable onDragStart={() => setDragEmployee(emp.name)} className="cursor-grab rounded-xl border bg-white px-3 py-2 shadow-sm"><div className="font-black">{emp.name}</div><div className="text-xs text-slate-500">{emp.hours ? `${emp.hours} h/Woche` : "Chef"}</div></div>)}
                  </div></div>
                  <h4 className="mb-2 font-black">Status</h4>
                  <div className="max-h-[520px] space-y-1 overflow-auto">
                    {PERSONNEL_EMPLOYEES.map((emp) => { const status = employeeStatusLabel(emp.name); const cls = status === "anwesend" ? "bg-green-50 border-green-200" : status === "krank" ? "bg-red-50 border-red-200" : status === "urlaub" ? "bg-yellow-50 border-yellow-200" : "bg-slate-100 border-slate-200"; return <button key={emp.name} type="button" onClick={() => cycleEmployeeStatus(emp.name)} className={`grid w-full grid-cols-[1fr_auto] rounded-lg border px-2 py-1 text-left text-sm ${cls}`}><span className="font-semibold">{emp.name}</span><span className="text-xs">{status}</span></button>; })}
                  </div>
                </aside>
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  {PERSONNEL_SECTIONS.map((section) => { const target = targetForSection(section); const assigned = employeesInSection(section.name); return <div key={section.name} onDragOver={(e) => e.preventDefault()} onDrop={() => dragEmployee && setEmployeeToSection(dragEmployee, section.name)} className={`min-h-[160px] rounded-3xl border-4 p-4 ${sectionColor(section)}`}><div className="mb-3 flex items-start justify-between gap-2"><div><h3 className="text-xl font-black">{section.name}</h3><div className="text-sm font-bold">Soll: {target === null ? "bei Bedarf" : target} | Ist: {assigned.length}</div></div><div className="text-2xl">{target === null ? "⚪" : assigned.length < target ? "🔴" : assigned.length > target ? "🟡" : "🟢"}</div></div><div className="space-y-2">{assigned.map((name) => { const emp = PERSONNEL_EMPLOYEES.find((e) => e.name === name); return <div key={name} draggable onDragStart={() => setDragEmployee(name)} className="cursor-grab rounded-xl border bg-white px-3 py-2 shadow-sm"><div className="font-black">{name}</div><div className="text-xs text-slate-500">{emp?.hours ? `${emp.hours} h/Woche` : "Chef"}</div></div>; })}</div></div>; })}
                </div>
              </div>
            </div>
          </section>
