...
function App() {
  ...
  async function archiveFinishedOrders(orderIds = null) {
    ...
    const containerCount = window.prompt("Containeranzahl oder Packerlanzahl eingeben:");
    if (containerCount === null) return;

    const tourNumber = window.prompt("Tourennummer eingeben:");
    if (tourNumber === null) return;

    await supabase
      .from("orders")
      .update({
        status: "auf_tour",
        container_count: containerCount.trim(),
        tour_number: tourNumber.trim(),
      })
      .in("id", ids);
