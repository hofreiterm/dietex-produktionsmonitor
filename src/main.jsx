...
export default function App() {
  ...
  const protectedView = view === "leitung" || view === "stats";
  ...
  async function importCustomersExcel(event) {
    ...
    try {
      ...
      const { error } = await supabase
        .from("customers")
        .upsert(imported, { onConflict: "customer_number" });
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="border-b bg-white px-8 py-4">
        <div className="grid grid-cols-3 items-center">
          ...
          <div className="flex items-center justify-end gap-3"><span className="text-2xl font-bold">◷ {fmtTime(new Date())}</span>{session && <Button onClick={logout}>Logout</Button>}</div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-[1800px] p-5">
        {!fixedView && (
          <nav className="mb-5 flex flex-wrap justify-center gap-2">
            {[["uebernahme", "Übernahme"], ["station", "Station"], ["monitor", "Verpackungsmonitor"], ["luftbild", "Luftbild"], ["stats", "Statistiken"], ["stammdaten", "Stammdaten"], ["leitung", "Produktionsleitung"]].map(([key, label]) => (
          ...
          <section className="rounded-3xl border bg-white p-4 shadow-sm">
            <div className="mb-3 flex justify-end">
              ...
              <label
                ...
                className="cursor-pointer rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-800 hover:bg-blue-100"
              >
        ...
        {view === "stammdaten" && (
          <section className="rounded-3xl border bg-white p-6 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-3">
              ...
              <Input placeholder="Kunde suchen" value={customerSearch} onChange={(e) => setCustomerSearch(e.target.value)} />
            </div>
