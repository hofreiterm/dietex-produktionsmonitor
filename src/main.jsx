...
function App() {
  ...
  async function archiveFinishedOrders(orderIds = null) {
    const finished = orders.filter((o) => {
      const related = items.filter((i) => i.order_id === o.id);
  ...
  async function toggleItem(item) {
    ...
    const orderItems = items
      ...
      .map((i) => (i.id === item.id ? { ...i, is_done: next } : i));
              ...
              {takenRows.map((row) => (
                <div key={row.id} className="rounded-xl border border-blue-200 bg-white p-3 shadow-sm">
                  ...
                  <div className="mt-2 flex gap-2 text-xl">
                    {["Bettwäsche", "Frottee", "Tischwäsche", "Putzerei"].map((cat) => (
                      <span key={cat} className={row.categories.includes(cat) ? "opacity-100" : "opacity-15"} title={cat}>{CAT_ICON[cat]}</span>
                    ))}
                  </div>
          ...
          <section className="grid gap-5 lg:grid-cols-4">
            {splitIntoColumns(workingRows, 2).map((col, idx) => (
              <div key={`work-${idx}`} className="rounded-3xl border border-orange-100 bg-orange-50/40 p-4">
                ...
                <div className="space-y-2">
                  {col.map((r) => <SmallCustomerCard key={r.id} row={r} />)}
                </div>
              </div>
            ))}
            {splitIntoColumns(finishedRows, 2).map((col, idx) => (
              <div key={`done-${idx}`} className="rounded-3xl border border-green-100 bg-green-50/40 p-4">
                ...
                <div className="space-y-2">
                  {col.map((r) => (
                    <SmallCustomerCard key={r.id} row={r} onClick={() => archiveFinishedOrders([r.id])} />
                  ))}
                </div>
              </div>
            ))}
        ...
        {view === "luftbild" && (
          <section className="rounded-3xl border bg-white p-5">
            <div className="mb-4 flex justify-between">
              <div>
                <h2 className="text-2xl font-black">Luftbild Container</h2>
                <p className="text-slate-500">Gelb = in Bearbeitung, Grün = fertig</p>
              </div>
        ...
        {view === "stats" && (
          <section className="rounded-3xl border bg-white p-6">
            <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
              <div>
                <h2 className="text-2xl font-black">Statistiken</h2>
                <p className="text-slate-500">Verpackungszeit je Kunde</p>
              </div>
          ...
          <section className="rounded-3xl border bg-white p-6 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-slate-500">Kunden suchen und einzelne Kunden löschen.</p>
              </div>
            ...
            <div className="max-h-[650px] overflow-auto rounded-2xl border">
              <table className="w-full text-left text-sm">
                <thead className="sticky top-0 bg-slate-100">
                  <tr><th className="p-3">Kundennummer</th><th className="p-3">Name</th><th className="p-3 text-right">Aktion</th></tr>
                </thead>
                <tbody>
                  {customers
                    .filter((c) => !masterSearch || c.customer_number.includes(masterSearch) || c.customer_name.toLowerCase().includes(masterSearch.toLowerCase()))
                    .map((c) => (
                      <tr key={c.id} className="border-t">
                        <td className="p-3 font-semibold">{c.customer_number}</td>
                        <td className="p-3">{c.customer_name}</td>
                        <td className="p-3 text-right">
                          <Button className="border-red-200 bg-red-50 text-red-700" onClick={() => deleteMasterCustomer(c)}>Löschen</Button>
                        </td>
                      </tr>
                    ))}
                </tbody>
