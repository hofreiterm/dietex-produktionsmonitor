...
function App() {
    ...
    setHiddenStationOrders((prev) => {
      ...
      orders.forEach((order) => {
        STATIONS.forEach((station) => {
          ...
          if (relevant.length > 0 && relevant.every((i) => i.is_done) && !next[key]) next[key] = Date.now() + 10000;
        });
      });
      return next;
    });
  ...
  async function importCustomersExcel(event) {
    ...
    try {
      ...
      const imported = rows
        .slice(1)
        .map((row) => ({ customer_number: String(row[0] || "").trim(), customer_name: String(row[1] || "").trim() }))
  ...
  function findFreeSlots(count) {
    for (const row of ROWS) {
      ...
      if (start + count - 1 <= PLACES) return Array.from({ length: count }, (_, i) => ({ row, place: start + i }));
    }
    return null;
  }
  ...
  async function addOrder() {
    ...
    if (!customer) {
      const { data, error } = await supabase.from("customers").insert({ customer_number: customerNumber.trim(), customer_name: customerName.trim() }).select().single();
    ...
    if (allDone) {
      ...
      setHiddenStationOrders((p) => {
        ...
        STATIONS.forEach((station) => {
          ...
          if (relevant.length > 0 && relevant.every((i) => i.is_done)) nextHidden[`${order.id}-${station.key}`] = Date.now() + 10000;
        });
        return nextHidden;
      });
    }
  ...
  async function deleteMasterCustomer(customer) {
    ...
    if (error) return alert("Kunde konnte nicht gelöscht werden: " + error.message);
    loadAll();
  }
  ...
  const monitorRows = useMemo(() => {
    return orders.map((order) => {
      ...
      if (total > 0 && done === total) monitorState = "fertig";
      else if (done > 0) monitorState = "bearbeitung";
      return { ...order, progressDone: done, progressTotal: total, monitorState, categories: getOrderCategories(order.id) };
  ...
  const stationOrders = useMemo(() => {
    return [...orders].sort((a, b) => String(a.customer_number).localeCompare(String(b.customer_number), "de", { numeric: true })).filter((order) => {
  ...
  }, [orders, items, articleSettings, activeStation, stationSearch, hiddenStationOrders, tick]);
  ...
  function SmallCustomerCard({ row, onClick }) {
    return (
      <button type="button" onClick={onClick} className={`grid w-full grid-cols-[85px_1fr_55px] items-center rounded-lg border bg-white px-3 py-2 text-left text-sm hover:shadow ${onClick ? "cursor-pointer" : ""}`}>
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="border-b bg-white px-8 py-4">
        <div className="grid grid-cols-3 items-center">
          ...
          <div className="flex items-center justify-end gap-3"><span className="text-2xl font-bold">◷ {fmtTime(new Date())}</span></div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-[1800px] p-5">
        {!fixedView && (
          <nav className="mb-5 flex flex-wrap justify-center gap-2">
            {[["annahme", "Kunden übernehmen"], ["uebernahme-monitor", "Übernahme"], ["station", "Station"], ["monitor", "Verpackungsmonitor"], ["luftbild", "Luftbild"], ["stats", "Statistiken"], ["stammdaten", "Stammdaten"], ["leitung", "Produktionsleitung"]].map(([key, label]) => (
        ...
        {view === "annahme" && (
          <section className="rounded-3xl border bg-white p-4 shadow-sm">
            <div className="mb-3 flex justify-end">
              ...
              <label htmlFor="customer-excel-import" className="cursor-pointer rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-800 hover:bg-blue-100">📄 Kunden Excel importieren</label>
            </div>
            ...
            {customerSuggestions.length > 0 && (
              <div className="mb-3 rounded-xl border bg-white">
                {customerSuggestions.map((c) => (
                  <button key={c.id} type="button" className="grid w-full grid-cols-[120px_1fr] border-b p-2 text-left" onClick={() => { setCustomerNumber(c.customer_number); setCustomerName(c.customer_name); setCustomerSearch(`${c.customer_number} ${c.customer_name}`); }}>
            ...
            <div className="grid gap-5 md:grid-cols-4">
              {Object.keys(CATEGORIES).map((cat) => (
                <button key={cat} type="button" onClick={() => setSelectedCategories((p) => (p.includes(cat) ? p.filter((x) => x !== cat) : [...p, cat]))} className={`rounded-3xl border-4 p-4 text-center transition-all ${categoryStyle(cat, selectedCategories.includes(cat))}`}>
            ...
            <div className="mt-5 flex justify-center border-t pt-4">
              <button type="button" onClick={addOrder} className="rounded-2xl bg-blue-700 px-8 py-3 text-base font-black text-white shadow-lg hover:bg-blue-800">Kunde übernehmen</button>
            </div>
          </section>
        )}

        {view === "uebernahme-monitor" && (
          <section className="rounded-3xl border border-blue-100 bg-blue-50/40 p-5">
            <h2 className="mb-4 border-b-2 border-blue-400 pb-2 text-center text-2xl font-black">ÜBERNAHME</h2>
            <div className="grid gap-3 md:grid-cols-4">
              {takenRows.map((row) => (
                <div key={row.id} className="rounded-xl border border-blue-200 bg-white p-3 shadow-sm">
                  <div className="grid grid-cols-[80px_1fr] gap-2"><span className="font-semibold">{row.customer_number}</span><b className="truncate">{row.customer_name}</b></div>
                  <div className="mt-2 flex gap-2 text-xl">
                    {["Bettwäsche", "Frottee", "Tischwäsche", "Putzerei"].map((cat) => <span key={cat} className={row.categories.includes(cat) ? "opacity-100" : "opacity-15"} title={cat}>{CAT_ICON[cat]}</span>)}
                  </div>
            ...
            <div className={`rounded-3xl border bg-white p-4 ${fixedView ? "lg:col-span-2" : ""}`}>
              ...
              <div className="grid gap-2 md:grid-cols-3 xl:grid-cols-4">
                {stationOrders.map((order) => {
                  const relevant = enabledItemsForOrder(order).filter((i) => activeStation.items.includes(i.subcategory)).sort((a, b) => activeStation.items.indexOf(a.subcategory) - activeStation.items.indexOf(b.subcategory));
        ...
        {view === "stammdaten" && (
          <section className="rounded-3xl border bg-white p-6 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <h2 className="text-2xl font-black">Stammdaten Kunden</h2>
                <p className="text-slate-500">Pro Kunde festlegen, welche Unterkategorien bei den Stationen sichtbar sind.</p>
              </div>
        ...
        {view === "leitung" && (
          <section className="space-y-5">
            <div className="grid gap-5 md:grid-cols-4">
              <div className="rounded-2xl border bg-white p-5"><small>Übernommen</small><div className="text-3xl font-black">{takenRows.length}</div></div>
