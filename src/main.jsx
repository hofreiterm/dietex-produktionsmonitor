  ...
  function SmallCustomerCard({ row, onClick }) {
    return (
      <button
        type="button"
        onClick={onClick}
        className={`grid w-full grid-cols-[105px_1fr_55px] items-center rounded-lg border bg-white px-3 py-2 text-left text-sm hover:shadow ${
          onClick ? "cursor-pointer" : ""
        }`}
      >
        <span>{row.customer_number}</span>
        <b className="text-[19px] leading-tight break-words whitespace-normal">{row.customer_name}</b>
  ...
  function WashCard({ row, category }) {
    return (
      <button
        ...
        className={`w-full rounded-xl border bg-white p-3 text-left shadow-sm hover:ring-2 hover:ring-blue-300 ${dragOrderId === row.id ? "opacity-40" : ""}`}
      >
        <div className="grid grid-cols-[28px_70px_1fr_42px] items-center gap-2">
        ...
        <div className="mt-2 text-xs font-semibold text-slate-500">Ziehen zum Reihnen • Antippen = gewaschen</div>
      </button>
    );
  }
